...
2. 同时修改 sw.js 里的 CACHE 名称（如 kitchen-assistant-v9 → kitchen-assistant-v10）。
3. 重新打包到以新版本号命名的文件夹（如 ../1.12.0/）并推送部署。
