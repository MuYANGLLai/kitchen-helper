# 厨房小助手 v1.2.0（PWA 部署包）

可直接部署的静态 PWA，无需构建。上传到任意静态托管（GitHub Pages / Vercel / Netlify 等）即可用手机访问并「添加到主屏幕」安装。

## 目录结构
index.html / manifest.webmanifest / sw.js / version.json / css/ / js/ / icons/

## 部署到 GitHub Pages
1. 新建 GitHub 仓库（如 kitchen-assistant）。
2. 把本文件夹（1.2.0）内所有内容推送到仓库根目录。
...
2. 同时修改 sw.js 里的 CACHE 名称（如 kitchen-assistant-v2 → kitchen-assistant-v3）。
3. 重新打包到以新版本号命名的文件夹（如 ../1.3.0/）并推送部署。
